...
<Tabs items={['Docker Deployment', 'Bare-metal Deployment', 'Kubernetes Deployment']} storageKey="selected-installation-method">
  <Tab>
    Prepare a configuration file `config.toml` in the current directory with the following content:

    ```toml copy filename="config.toml"
    # Configure the evaluation system to run an Http service on port 8080 to receive our evaluation tasks
    [exchange.demo]
    type = "http"
    address = "0.0.0.0"
    port = 8080

    # The security sandbox needs to know which user on the system to use to create containers
...
We've prepared a simple evaluation task that adds a `main.py` source file and uses the official Python image to run the source file and get the output.
...
Using a tool like `curl`, we can send a request containing the evaluation task to Seele's Http service:

```sh copy
curl --data-binary @task.yaml http://127.0.0.1:8080
```

After Seele receives this evaluation task, it will attempt to connect to [Docker Hub](https://hub.docker.com) to download the official Python image and run the task.
...
- If you want to learn more about Seele's evaluation tasks, please read [Evaluation Tasks](/tasks/description).
- If you want to review Seele's various configuration options, please read [Configuration Items](/configurations/file).
