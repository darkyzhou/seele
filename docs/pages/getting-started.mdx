...
<Tabs items={['Docker 部署', '裸机部署', 'Kubernetes 部署']} storageKey="selected-installation-method">
  <Tab>
    使用下列命令拉取 Seele 的最新版容器镜像：

    ```sh
    docker pull ghcr.io/darkyzhou/seele
    ```

    在当前目录中准备一份配置文件 `config.toml`，内容如下：
    ...
    <Callout type="info">
      请尽可能使用最新版的 Docker 以避免出现功能不支持的问题。
    </Callout>
...
- 如果你想进一步了解 Seele 的评测任务，请阅读[评测任务](/tasks/types)。
- 如果你想查阅 Seele 的各种配置项，请阅读[配置项](/configurations/file)。
- 如果你对 Seele 的技术细节感兴趣，请阅读[技术细节](/technical-details/architecture)。
